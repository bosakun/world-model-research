# Heterogeneous Simulator: 異なるsimulatorデータを安全に統合する
...
simulatorごとにobservation、action、座標系、単位が異なり、数値だけを混ぜると意味が壊れます。
...
shapeが同じでも意味が違う場合があり、形式の統一だけではsemantic alignmentになりません。
