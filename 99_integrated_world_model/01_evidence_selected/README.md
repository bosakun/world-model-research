...
このページは最後のLessonです。RSSM、ensemble、MPCを一度に理解しようとしないでください。まず次の一本の流れだけを追います。

```text
画像を見る -> 過去を思い出す -> 未来を想像する -> 良さそうなactionを一つ選ぶ
```

その後に「なぜ画像を見る前のpriorだけで未来を想像するのか」「なぜ毎action後に考え直すのか」を読めば十分です。
