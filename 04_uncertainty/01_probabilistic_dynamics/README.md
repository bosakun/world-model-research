...
この図で、`[2]`は「数字が2個」、`[4]`は「数字が4個」というshapeです。`+`は足し算ではなく、情報を横へつなげることを表します。

1. `state s_t [2]`は現在位置です。例えば`[0.3, 0.7]`なら、横位置`x`と縦位置`y`の二つの数字です。
2. `one-hot action a_t [4]`は、left/right/down/upのどれを選んだかです。rightなら`[0, 1, 0, 0]`のように、一つだけを`1`にします。
3. 二つをつなげた6個の数字をMLP（普通のニューラルネットワーク）へ入れます。MLPは「この位置でこのactionをすると、次はどう動くか」を学びます。
4. 左branchの`mean delta [2]`は平均の変化量です。例えば`[0.1, 0.0]`なら、平均では横に`+0.1`動くという意味です。現在位置へ足すと、次stateの中心`mu`になります。
5. 右branchの`raw log variance [2]`は、横方向・縦方向それぞれで結果がどれくらい揺れそうかを作るための値です。負のvarianceを出さないため、modelは標準偏差そのものではなくlog varianceを出します。
6. `Gaussian N(mu, diag(sigma^2))`は、「中心は`mu`だが、その周りに`sigma`の広がりがある」という次stateの分布です。`diag`は横と縦の揺れを別々に扱い、今回は両者の相関を扱わないという意味です。
7. 最後に`mu`だけを使えば最も中心的な未来を得ます。Gaussianからsampleすれば、同じactionでも起こりうる異なる未来候補を得ます。

例えば、現在位置`[0.3, 0.7]`でrightを選び、`mean delta=[0.1, 0.0]`なら、平均の次位置は`mu=[0.4, 0.7]`です。`sigma=[0.05, 0.10]`なら、横には少し、縦にはより大きく揺れうるという予測になります。
