...
この実験の答えは、「同じ現在画像でも、過去に見たGoalが違えば正しい行動は変わる」である。`outputs/aliasing_pair.png`を先に見てから[UNDERSTANDING.md](UNDERSTANDING.md)のLesson 2を読む。

以下は、その状態をプログラムとして再現・検査するための技術記録である。
